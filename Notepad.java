...
public class Notepad extends JFrame implements ActionListener,DocumentListener  {
   ...
   JLabel statusLabel;  
   //系统剪贴板
   //Toolkit用于将各种组件绑定到特定本机工具包实现，Toolkit.getDefaultToolkit()获取默认工具
   Toolkit toolkit=Toolkit.getDefaultToolkit();  
   //Clipboard实现一种使用剪切/复制/粘贴操作传输数据的机制
   Clipboard clipBoard=toolkit.getSystemClipboard();  
   //创建撤销操作管理器(与撤销操作有关)  
   //UndoManager管理 UndoableEdit列表，提供撤消或恢复适当编辑的方法。
   protected UndoManager undo=new UndoManager();  
   //有两个方法可以将编辑添加到 UndoManager。直接使用 addEdit方法添加编辑，或将 UndoManager添加到支持 UndoableEditListener的 bean
...
//       测试此枚举是否包含更多的元素。
   ...
   {   final JDialog fontDialog=new JDialog(this,"字体设置",false);  
       ...
       final JLabel sample=new JLabel("我的记事本-ZHG's Notepad");  
